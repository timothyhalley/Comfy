...
async def download_model(
    ...
    if not validate_filename(model_name):
        return DownloadModelStatus(
            DownloadStatusType.ERROR, 0, "Invalid model name", False
        )
    ...
    try:
        ...
        status = DownloadModelStatus(
            DownloadStatusType.PENDING, 0, f"Starting download of {model_name}", False
        )
        ...
        if response.status != 200:
            error_message = (
                f"Failed to download {model_name}. Status code: {response.status}"
            )
            logging.error(error_message)
            status = DownloadModelStatus(
                DownloadStatusType.ERROR, 0, error_message, False
            )
            await progress_callback(model_name, status)
            return DownloadModelStatus(
                DownloadStatusType.ERROR, 0, error_message, False
        ...
        return await track_download_progress(
            response, file_path, model_name, progress_callback, progress_interval
        )
...
async def check_file_exists(
    ...
    if os.path.exists(file_path):
        status = DownloadModelStatus(
            DownloadStatusType.COMPLETED, 100, "{model_name} already exists", True
        )
        await progress_callback(model_name, status)
        return status
    return None
...
async def track_download_progress(
    ...
    try:
        ...
        async def update_progress():
            ...
            status = DownloadModelStatus(
                DownloadStatusType.IN_PROGRESS,
                progress,
        ...
        logging.info(
            f"Successfully downloaded {model_name}. Total downloaded: {downloaded}"
        )
        status = DownloadModelStatus(
            DownloadStatusType.COMPLETED,
            100,
